...
public class NumSubarrayProductLessThanK {
    ...
    public static void main(String[] args) {
       ...
       /* int[] nums = {10,9,10,4,3,8,3,3,6,2,10,10,9,3};
        ...
        //System.out.println(calculateProd(nums));
    }
    ...
    private static BigInteger calculateProd(int[] nums) {
   ...
   /* private static int numSubarrayProductLessThanK(int[] nums, int k) {
        ...
        for(int i =1; i<=len;i++){
            for(int j=0;j<len;j++){
                ...
                for(int x = j;x<m;x++){
                    //prod *=nums[x];
                    prod = prod.multiply(BigInteger.valueOf(nums[x]));
                }
                if(prod.compareTo(BigInteger.valueOf(k))==-1){
                    count++;
                }
